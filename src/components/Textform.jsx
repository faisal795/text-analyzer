...
function Textform(props) {
    ...
    let wordCounter = myText.trim().split(" ").filter((myText) => myText !== "").length;
    let SPACECOUNTER; (wordCounter === 1) ? SPACECOUNTER = 0 : SPACECOUNTER = wordCounter;

    //  const paragraphs = myText.split(/\r?\n|\r/);
    ...
    return (
        <>
            <div className="container py-4">
                ...
                <div className="mb-3">
                    <label htmlFor="myBox" className="form-label">Type the text blew here!</label>
                ...
                <button className='btn btn-outline-success m-2' onClick={convertUppercase}>Uppercase Text</button>
                <button className='btn btn-outline-info m-2' onClick={convertLowerCase}>LowerCase Text</button>
                <button className='btn btn-outline-danger m-2' onClick={clearAllText}>Clear Text</button>
                <button className='btn btn-outline-warning m-2' onClick={capitalizeText}>Firt chr Upper</button>
                <button className='btn btn-outline-primary m-2' onClick={copyText}>Copy Text</button>
                <button className='btn btn-outline-secondary m-2' onClick={removeSpaces}>Remove Extr Spaces</button>
            ...
            <div className="container">
                ...
                <div className="row text-white">
                    ...
                    <div className="col m-1 bg-dark"><p>CHARACTERS WITHOUT SPACE: <span>{myText.length - myText.split(" ").filter((myText) => myText !== "").length}</span></p></div>
                ...
                <div className="row py-3">
                    ...
                    <div className="col">
                        <p className='lead'>{myText.length > 0 ? myText : 'Enter something in the above textarea and preview it here!'}</p>
                    </div>
                </div>
            </div>
